...
#     print(cm_size_y, cm_size_x)


class InterrogationWindowSetting:
    def __init__(
        self,
        image_width,
        image_height,
        ...
        image_height: 画像の高さ
        iw_size: 検査領域サイズ
        image: 画像（次元: [1, 1, img_width, img_height]）
        """
        ...
        self.stride = int(self.iw_size * (1 - self.overlap))
        self.device = device

        # 検査画像を生成
        self.interrogation_image = (
            (
                image[
                    0:1,
                    0:1,
                ...
                .unfold(2, iw_size, self.stride)
                .unfold(3, iw_size, self.stride)
            )
            .permute(2, 3, 1, 4, 5, 0)
            .squeeze(-1)
        )  # 次元: [ny, nx, 1, iw_size, iw_size]

        # if FLAG_DEBUG:
        #     check_variable("self.interrogation_image", self.interrogation_image)

        # 速度定義点の数
        self.ny = self.interrogation_image.shape[0]
        self.nx = self.interrogation_image.shape[1]

        # デバイス設定
        self.device = self.interrogation_image.device
    ...
    def get_image(self):
        """
        検査画像テンソル（次元: [ny, nx, 1, iw_size, iw_size]）を返す
        """
        return self.interrogation_image

    def get_iw_position(self):
        """
        各検査画像の左上と右下のピクセル位置座標を返す
        """
        # インデックスグリッドの生成（ny, nx）
        ...
        # ピクセル位置の計算
        # 左上
        lt_y = self.margin + grid_j * self.stride
        lt_x = self.margin + grid_i * self.stride
        # 右下
        rb_y = lt_y + self.iw_size - 1
        rb_x = lt_x + self.iw_size - 1

        iw_position_lt = torch.stack([lt_y, lt_x], dim=-1)
        iw_position_rb = torch.stack([rb_y, rb_x], dim=-1)

        return iw_position_lt, iw_position_rb


class SearchWindowSetting:
    def __init__(
        self,
        image_width,
        image_height,
        ...
        image_height: 画像の高さ
        image: 画像（次元: [1, 1, img_width, img_height]）
        iw_position_lt: 各検査画像の左上位置
        offset: 各探査領域のオフセット変位（次元: []）
        iw_size: 検査領域サイズ
        sw_size: 探査領域サイズ
        """
        ...
        self.image_height = image_height
        self.image = image
        self.iw_position_lt = iw_position_lt
        self.ny = self.iw_position_lt.shape[0]
        self.nx = self.iw_position_lt.shape[1]
        self.device = device
        if offset == None:
            self.offset = torch.zeros(
                self.ny, self.nx, 2, dtype=torch.long, device=self.device
            )
        else:
            self.offset = offset
        self.iw_size = iw_size
        ...
        # オフセットがない場合，検査領域と探査領域の中心を合わせるためのシフト量
        shift = (self.sw_size - self.iw_size) // 2

        # 探査領域のピクセル位置の計算
        lt_y = self.iw_position_lt[..., 0] - shift + self.offset[..., 0]
        lt_x = self.iw_position_lt[..., 1] - shift + self.offset[..., 1]

        # 探査領域が必ず画像内に収まるように修正
        lt_y = torch.clamp(lt_y, min=0, max=self.image_height - self.sw_size)
        lt_x = torch.clamp(lt_x, min=0, max=self.image_width - self.sw_size)
        self.sw_position_lt = torch.stack([lt_y, lt_x], dim=-1)
        self.sw_position_rb = self.sw_position_lt + sw_size - 1

    def get_sw_position(self):
        """
        探査画像のピクセル位置座標を返す
        """
        return self.sw_position_lt, self.sw_position_rb

    def get_sw_image(self):
        """
        探査画像テンソル（次元: [ny, nx, 1, sw_size, sw_size]）を返す
        """
        ...
        # if FLAG_DEBUG:
        #     check_variable("rel_grid_x", rel_grid_x)

        # 探査画像の各ピクセルの絶対座標を計算
        abs_y = (self.sw_position_lt[..., 0, None, None] + rel_grid_y).reshape(
            self.ny * self.nx, self.sw_size, self.sw_size
        )
        abs_x = (self.sw_position_lt[..., 1, None, None] + rel_grid_x).reshape(
            self.ny * self.nx, self.sw_size, self.sw_size
        )
        # if FLAG_DEBUG:
        #     check_variable("abs_x", abs_x)

        # ２次元座標を１次元座標に変換
        idx_1d = (abs_y * self.image_width + abs_x).reshape(-1)
        # idx_1d = abs_y * self.image_width + abs_x
        # if FLAG_DEBUG:
        #     check_variable("idx_1d", idx_1d)

        # 画像を一次元化
        image_flatten = self.image.reshape(-1)

        # 一括サンプリング
        self.search_image = torch.gather(image_flatten, dim=0, index=idx_1d).reshape(
            self.ny, self.nx, 1, self.sw_size, self.sw_size
        )
        # if FLAG_DEBUG:
        #     check_variable("search_image", self.search_image)

        return self.search_image


class PatternMatchSetting:
    def __init__(
        self, nx, ny, iw_size, interrogation_image, sw_size, search_image, device
    ):
        """
        画像相関を計算するクラス
        検査画像テンソル: interrogation_image（次元: [ny, nx, 1, iw_size, iw_size]）
        探査画像テンソル: search_image（次元: [[ny, nx, 1, sw_size, sw_size]]）
        """
        ...
        self.ny = ny
        self.iw_size = iw_size
        self.interrogation_image = interrogation_image
        self.sw_size = sw_size
        self.search_image = search_image
        self.device = device

        if FLAG_DEBUG:
            check_variable("interrogation_image", interrogation_image)
            check_variable("search_image", search_image)

        # 相互相関係数の計算
        self.n = self.nx * self.ny  # 画像総数
        self.interrogation_image_flatten = self.interrogation_image.reshape(
            self.n, 1, self.iw_size, self.iw_size
        )  # 検査画像の平坦化
        self.interrogation_image_mean = torch.mean(
            self.interrogation_image_flatten, dim=(-2, -1), keepdim=True
        )  # 各検査画像の平均を計算
        self.interrogation_image_flatten = (
            self.interrogation_image_flatten - self.interrogation_image_mean
        )  # 各検査検査の平均を0に

        self.search_image_flatten = self.search_image.reshape(
            1, self.n, self.sw_size, self.sw_size
        )  # 探査画像の平坦化

        # 相関係数の分子の計算
        self.cm_size = self.sw_size - self.iw_size + 1
        self.correlation_coef_nume = F.conv2d(
            self.search_image_flatten, self.interrogation_image_flatten, groups=self.n
        ).reshape(self.ny, self.nx, self.cm_size, self.cm_size)

        if FLAG_DEBUG:
            check_variable("self.correlation_coef_nume", self.correlation_coef_nume)

        # 相関係数の分母の計算
        # 検査画像の標準偏差
        self.interrogation_image_flatten_sq_sum = torch.sum(
            self.interrogation_image_flatten**2, dim=(-2, -1), keepdim=True
        )
        self.interrogation_image_flatten_std = torch.sqrt(
            torch.clamp(self.interrogation_image_flatten_sq_sum, min=1e-8)
        )
        if FLAG_DEBUG:
            check_variable(
                "self.interrogation_image_flatten_std",
                self.interrogation_image_flatten_std,
            )
        # 探査画像の標準偏差
        self.ones_filter = torch.ones(
            self.n, 1, self.iw_size, self.iw_size, device=self.device
        )
        self.search_image_flatten_sum = F.conv2d(
            self.search_image_flatten, self.ones_filter, groups=self.n
        )  # 和
        self.search_image_flatten_sq_sum = F.conv2d(
            self.search_image_flatten**2, self.ones_filter, groups=self.n
        )  # ２乗和
        self.search_image_flatten_var = (
            self.search_image_flatten_sq_sum
            - self.search_image_flatten_sum**2 / (self.iw_size * self.iw_size)
        )
        self.search_image_flatten_std = torch.sqrt(
            torch.clamp(self.search_image_flatten_var, min=1e-8)
        )
        if FLAG_DEBUG:
            check_variable(
                "self.search_image_flatten_std", self.search_image_flatten_std
            )
        self.correlation_coef_deno = (
            self.interrogation_image_flatten_std.reshape(1, self.n, 1, 1)
            * self.search_image_flatten_std
        ).reshape(self.ny, self.nx, self.cm_size, self.cm_size)

        if FLAG_DEBUG:
            check_variable("self.correlation_coef_deno", self.correlation_coef_deno)

        self.correlation_coef = self.correlation_coef_nume / self.correlation_coef_deno

        # 範囲を-1~1に修正
        self.correlation_coef = torch.clamp(self.correlation_coef, min=-1, max=1)

        # 相関が最大となる座標を計算
        correlation_coef_flatten = self.correlation_coef.reshape(self.ny, self.nx, -1)

        if FLAG_DEBUG:
            check_variable("correlation_coef_flatten", correlation_coef_flatten)

        max_vals, max_pos = torch.max(correlation_coef_flatten, dim=-1)

        if FLAG_DEBUG:
            check_variable("max_vals", max_vals)
            check_variable("max_pos", max_pos)

        # 最大値での座標を計算
        peak_y = max_pos // self.cm_size
        peak_x = max_pos % self.cm_size

        self.peak_position = torch.stack([peak_y, peak_x], dim=-1)
        self.peak_value = max_vals

    def get_correlation_map(self):
        """
        相関係数テンソルを取得
        """
        return self.correlation_coef

    def get_peak_position(self):
        """
        相関係数テンソルの最大値とその位置を取得
        """

        return self.peak_value, self.peak_position
