...
class PivMatch:
    ...
    def get_peak_position(self, n_window, correlation_map):
        """
        相関係数配列のピークの位置を取得
        """
        # 相関係数配列の大きさ取得
        ...
        # 次元の調整
        correlation_map = correlation_map.reshape(n_window, -1)

        # 最大値とその位置の取得
        max_value, max_posit = torch.max(correlation_map, dim=-1)
