...
def plot_vector(n_x, n_y, iw_posit, vector, output_filepath=None):

    # 引数をnumpy配列に変換
    iw_posit = iw_posit.cpu().numpy().reshape((n_y, n_x, 4))
    print(iw_posit.shape)
    iw_center_y = (iw_posit[:, :, 0] + iw_posit[:, :, 2]) / 2
    iw_center_x = (iw_posit[:, :, 1] + iw_posit[:, :, 3]) / 2
    vector = vector.cpu().numpy().reshape((n_y, n_x, 2))
    ...
    # 描画範囲
    ax.set_xlim(np.min(iw_center_x), np.max(iw_center_x))
    ax.set_ylim(np.min(iw_center_y), np.max(iw_center_y))
