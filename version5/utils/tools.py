...
def plot_vector(n_x, n_y, iw_center, vector, output_filepath=None):

    # 引数をnumpy配列に変換
    if isinstance(iw_center, torch.Tensor):
        iw_center = iw_center.detach().cpu().numpy()

    if isinstance(vector, torch.Tensor):
        vector = vector.detach().cpu().numpy()

    # メッシュ作成
    axis_x = iw_center[0, :, 1]
    axis_y = iw_center[:, 0, 0]
    ...
    # 描画範囲
    ax.set_xlim(np.min(mesh_x), np.max(mesh_x))
    ax.set_ylim(np.min(mesh_y), np.max(mesh_y))
    ...
    if output_filepath is None:
        ...
        fig.savefig(fname=output_filepath, bbox_inches="tight", dpi=400)
        plt.close(fig)


# def output_coordinates(n_x, n_y):
