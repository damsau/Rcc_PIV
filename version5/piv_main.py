...
def piv_exe():
    ...
    for idx_frame in range(PivParams.START, PivParams.END + 1, PivParams.SKIP):
        ...
        # 流速のフィルター処理
        if velocity_backref.shape[0] == 0:
            velocity_all = velocity_forref
        elif velocity_forref.shape[0] == 0:
            velocity_all = velocity_backref
        else:
            velocity_all = np.concatenate([velocity_backref, velocity_forref], axis=0)

        # 速度の時間方向の中央値
        velocity_median = np.median(velocity_all, axis=0)

        tools.plot_vector(
            PivParams.N_X[PivParams.N_RCC - 1],
            PivParams.N_Y[PivParams.N_RCC - 1],
            PivWindow.IW_CENTER_LBO_YP[PivParams.N_RCC - 1] * PivParams.PIXEL_TO_MM,
            velocity_median,
            output_filepath=PICS_DIR / Path(f"velocity_target{idx_target}_median.png"),
        )
        ...
        output_velocity(
            PivParams.N_X[PivParams.N_RCC - 1],
            PivParams.N_Y[PivParams.N_RCC - 1],
            velocity_median.reshape(
                1, velocity_median.shape[0], velocity_median.shape[1], 2
            ),
            VELOCITY_DIR / Path(f"velocity_mmps_target{idx_target}_median.npy"),
        )
